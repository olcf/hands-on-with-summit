...
int main(int argc, char** argv) {
   ...
   printf ("Number in circle for %d rank: %ld \n", myRank, processNumberInCircle);
   finish = MPI_Wtime();
...
long Toss (long processTosses, int myRank){
        ...
        srand(seed + myRank);
        SCOREP_USER_REGION_DEFINE(toss_hdl)
        SCOREP_USER_REGION_BEGIN(toss_hdl, "user_reegion", SCOREP_USER_REGION_TYPE_LOOP)
        for (toss = 0; toss < processTosses; toss++) {
           x = rand_r(&seed)/(double)RAND_MAX;
           y = rand_r(&seed)/(double)RAND_MAX;
           if((x*x+y*y) <= 1.0 ) numberInCircle++;
    }
        SCOREP_USER_REGION_END(toss_hdl)
